...
@WebServlet(
        urlPatterns = {"/adminOnly/searchTournament"}
)
public class SearchTournament extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
        String searchTerm = req.getParameter("tournamentName");

        try {
            GenericDao tournamentDao = new GenericDao(Tournament.class);
            ObjectMapper jsonMapper = new ObjectMapper();

            List<Tournament> tournaments = tournamentDao.getByPropertyLike("name", searchTerm);
            String coursesJson = jsonMapper.writeValueAsString(tournaments);
